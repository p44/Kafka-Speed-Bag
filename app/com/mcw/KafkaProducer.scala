 ...
 * The default props are build using KSpeedBagModels.KAFKA_METADATA_BROKER_LIST
 *
 */
object KafkaProducer {
  ...
  def defaultProps: Properties = {
    ...
    p.put("metadata.broker.list", KSpeedBagModels.KAFKA_BROKER_LIST)
   ...
   * @return
   */
  def produceMessageToTopic(producer: Producer[AnyRef, AnyRef], topic: String, k: String, m: String): Boolean = {
    println(s"produceMessageToTopic ${topic}  ${m}")
    val keyed: KeyedMessage[AnyRef, AnyRef] = new KeyedMessage(topic, k.getBytes("UTF8"), m.getBytes("UTF8"))
  ...
  def sendMessage(producer: Producer[AnyRef, AnyRef], topic: String, m: String, keyed: KeyedMessage[AnyRef, AnyRef]):Boolean = {
    try {
      ...
      case t: Throwable => {
        println(s"There was a problem sending a message to kafka $topic $m")
        t.printStackTrace()
        false
      }
    }
